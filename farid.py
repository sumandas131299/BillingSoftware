...
class SecondUI(QWidget):
    ...
    def initUI(self):
       ...
       # Create a horizontal layout for Service and Plan Dropdowns
        ...
        # Checkbox to Enable/Disable Service Dropdown
        self.service_checkbox = QCheckBox("Enable Service Dropdown")
        self.service_checkbox.setChecked(True)  # Default is checked (dropdown enabled)
        self.service_checkbox.stateChanged.connect(self.toggle_service_input)
        service_plan_layout.addWidget(self.service_checkbox)


        # Custom Service Name Input Field
        self.custom_service_input = QLineEdit()
        self.custom_service_input.setPlaceholderText("Enter custom service name")
        self.custom_service_input.setEnabled(False)  # Initially disabled
        service_plan_layout.addWidget(self.custom_service_input)

        # Checkbox to Enable/Disable Custom Service Input
        self.custom_service_checkbox = QCheckBox("Enable Custom Service")
        self.custom_service_checkbox.stateChanged.connect(self.toggle_custom_service_input)
        service_plan_layout.addWidget(self.custom_service_checkbox)

        # Add the horizontal layout to the main layout
        layout.addLayout(service_plan_layout)

         # Create a vertical layout for Plan and Price Dropdowns
        plan_price_layout = QHBoxLayout()

        #  # Plan Dropdown
        self.plan_combo = QComboBox()
        self.plan_combo.currentTextChanged.connect(self.update_pricing_combo)
        plan_price_layout.addWidget(QLabel("Select Plan:"))
        plan_price_layout.addWidget(self.plan_combo)

        # Pricing Dropdown
        self.price_combo = QComboBox()
        plan_price_layout.addWidget(QLabel("Select Pricing:"))
        plan_price_layout.addWidget(self.price_combo)

        # Add the plan/price layout to the main layout
        layout.addLayout(plan_price_layout)
        ...
        self.template_preview_label.setFixedSize(350, 350)
    ...
    def toggle_service_input(self, state):
            """Enable or disable the service combo box based on the checkbox state."""
            if state == 2:  # Checkbox checked
                self.service_combo.setEnabled(True)  # Enable the service combo box
                self.custom_service_input.setEnabled(False)  # Disable custom service input
                self.custom_service_checkbox.setChecked(False)  # Uncheck the custom service checkbox
            else:  # Checkbox unchecked
                self.service_combo.setEnabled(False)  # Disable the service combo box
                self.custom_service_input.setEnabled(True)  # Enable custom service input
                self.custom_service_checkbox.setChecked(True)  # Check the custom service checkbox

    def toggle_custom_service_input(self, state):
        """Enable or disable the custom service input field based on checkbox state."""
        if state == 2:  # Checkbox checked
            self.service_combo.setEnabled(False)  # Disable the service combo box
            self.custom_service_input.setEnabled(True)  # Enable custom service input
            self.service_checkbox.setChecked(False)  # Uncheck the service dropdown checkbox
        else:  # Checkbox unchecked
            self.service_combo.setEnabled(True)  # Enable the service combo box
            self.custom_service_input.setEnabled(False)  # Disable custom service input
            self.service_checkbox.setChecked(True)  # Check the service dropdown checkbox
    ...
    def update_plan_combo(self, service_name):
        self.plan_combo.clear()  # Clear existing items
        if service_name:  # Check if a service is selected
            plans = self.services.get(service_name, {})  # Get plans for the selected service
            self.plan_combo.addItems(plans.keys())  # Add the plan names to the combo box
        else:
    ...
    def update_pricing_combo(self, plan_name):
        service = self.service_combo.currentText()
        self.price_combo.clear()  # Clear existing prices
        if service in self.services and plan_name in self.services[service]:
            prices = self.services[service][plan_name]
            self.price_combo.addItems(self.services[service][plan_name])  # Add prices for selected plan
            ...
            self.price_combo.clear()  # Clear pricing if no valid service/plan is found
            self.discounted_amount_input.clear()  # Clear discounted amount input if no valid pricing


    def add_service(self):
        # Check if the "Enter Custom Service" checkbox is checked
        if self.custom_service_checkbox.isChecked():  # Ensure this is the correct checkbox
            # Get the custom service name entered by the user
            custom_service = self.custom_service_input.text()
            if not custom_service:
                QMessageBox.warning(self, "Input Error", "Please enter a valid custom service name.")
                return
            selected_service = custom_service  # Use the custom service entered
        else:
            # If the custom checkbox is not checked, use the selected service from the dropdown
            ...
            if match:
                ...
                if self.enable_custom_amount_checkbox.isChecked():
                    ...
                    if self.enable_discount_checkbox.isChecked():
                        ...
                        discount_percentage = selected_discount
    ...
    def generate_invoice(self):
        ...
        for page_number, start_idx in enumerate(range(0, len(self.services_info), 8)):
            ...
            <tr>
              ...
              <tr>
                        <td><strong>Total : {total_installments or "0"}/-</strong></td>
                    </tr>
